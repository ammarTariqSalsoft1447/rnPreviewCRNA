...
import {
    View, TouchableOpacity, Animated, Dimensions, Keyboard
} from 'react-native'
...
export default class extends React.Component {
    ...
    show = (key, data, title, onSelect, onCancel, selectedIndex) => {
        ...
        if (Array.isArray(data)) {
            if (data.length > 0) {
                let selected = selectedIndex ? selectedIndex : Math.floor(data.length / 2)
                let dataToShow = []
                data.forEach(item => {
                    if (key != null) {
    ...
    render() {
        ...
        let animatedtitleBarStyle = {
            height: Animated.multiply(this.animatedBackdrop, new Animated.Value(0.07))
        }
        let animatedContainentStyle = {
            height: Animated.multiply(this.animatedBackdrop, new Animated.Value(0.33))
        }
        let animatedFont = {
            fontSize: Animated.multiply(this.animatedBackdrop, new Animated.Value(0.02))
        }
        return (
            <Animated.View activeOpacity={1} style={[styles.container, animatedBackdropStyle]}>
                <AnimatedTouchable onPress={this.cancel} style={[styles.backdropButton, animatedContainerButton]}>
                </AnimatedTouchable>
                <Animated.View style={[styles.conetntContainer, animatedContentContainerStyle]}>
                    <Animated.View style={[styles.titleBar, animatedtitleBarStyle]}>
                        <TouchableOpacity onPress={this.cancel} style={styles.titleContainer}>
                            <AmatedRegularText style={[styles.font, animatedFont]}>Cancel</AmatedRegularText>
                        </TouchableOpacity>
                        <View style={styles.titleContainer}>
                            <AmatedRegularText style={[styles.font, animatedFont]}>{this.state.title}</AmatedRegularText>
                        </View>
                        <TouchableOpacity onPress={this.done} style={styles.titleContainer}>
                            <AmatedRegularText style={[styles.font, animatedFont]}>Done</AmatedRegularText>
                        </TouchableOpacity>
                    </Animated.View>
